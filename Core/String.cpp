...
void String::Replace(char replaceThis, char replaceWith, bool caseSensitive)
{
    ...
    {
        replaceThis = KhSTL::ToLower(replaceThis);
        for (Iterator it = Begin(); it != End(); ++it)
        {
            if (KhSTL::ToLower(*it) == replaceThis)
                *it = replaceWith;
        }
    }
}
...
unsigned String::CaseInsensitiveHash(const char* str)
{
    ...
    while (*str)
    {
        hash = (KhSTL::ToLower(*str)) + (hash << 6) + (hash << 16) - hash;
        ++str;
    }
...
int String::Compare(const char* lhs, const char* rhs, bool caseSensitive)
{
    ...
    else
    {
        for (;;)
        {
            char l = KhSTL::ToLower(*lhs);
            char r = KhSTL::ToLower(*rhs);
            if (!l || !r)
                return l ? 1 : (r ? -1 : 0);
            if (l < r)
                return -1;
